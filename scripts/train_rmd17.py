...
from dataset.md17 import MD17, get_mean_std, get_dataloaders
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description='rMD17 Training Script')
    parser.add_argument('--config_path', type=str, default=root_dir + '/config/md17.yml',
    ...
    parser.add_argument('--num_train', type=int, default=950, help='Number of training samples.')
    parser.add_argument('--num_val', type=int, default=50, help='Number of validation samples.')
...
def load_config(config_path, args):
    ...
    if args.num_train is not None:
        config.data.num_train = args.num_train
        config.data.num_val = args.num_val
    if config.data.base_path is None:
        config.data.base_path = root_dir + '/data/rmd17'
...
def train_one_epoch(epoch, dataloader, model, optimizer, config, device, mean, std, partition='train'):
    ...
    for i, data in tqdm(enumerate(dataloader), unit='batch', mininterval=120):
        ...
        # Calculate losses
        loss_energy = F.l1_loss(pred, label) * config.train.kcal2mev
        loss_force = F.l1_loss(dy, pdy) * config.train.kcal2mev
        loss_momentum = F.l1_loss(total_force_pred, total_force) * config.train.kcal2mev  # Currently unused
...
def main():
    ...
    dataloaders = get_dataloaders(
        ...
        idx_dir=config.data.base_path,
    )
    mean, std = get_mean_std(dataloaders)

    # Initialize model
    # If molecule is aspirin, vec_norm can be set as True
    ...
    for epoch in range(config.train.epochs):
        train_loss, train_energy, train_force = train_one_epoch(
            epoch, dataloaders['train'], model, optimizer, config, device, mean, std, partition='train'
        )

        if epoch % config.test.test_interval == 0:
            val_loss, val_energy, val_force = train_one_epoch(
                epoch, dataloaders['val'], model, optimizer, config, device, mean, std, partition='valid'
            )
